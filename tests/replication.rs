...
async fn test_replication() -> anyhow::Result<()> {
	...
	assert_eq!(c6.get_local_rpc(context::current(), k2.clone()).await.unwrap().unwrap(), v2);
	assert_eq!(c0.get_local_rpc(context::current(), k2.clone()).await.unwrap().unwrap(), v2);
	...
	assert_eq!(c3.get_local_rpc(context::current(), k1.clone()).await.unwrap(), None);
	assert_eq!(c6.get_local_rpc(context::current(), k1.clone()).await.unwrap(), None);
